...
package Regions.Shared_Lists is
   ...
   function To_Index (Self : Cursor) return Positive with Inline;
...
private
   ...
   No_Element : constant Cursor := (Item => null, Index => 1);

   function To_Index (Self : Cursor) return Positive is (Self.Index);

   function Has_Element (Self : Cursor) return Boolean is (Self.Item /= null);
